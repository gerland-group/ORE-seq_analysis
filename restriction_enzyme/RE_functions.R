...
#'
#' @examples
save_bam_as_chr_df_list <- function (name, bam_folder = "", extension = ".bam", RData_folder = "", bam_duplicates_flag = NA) {
  ...
  fragments <- readGAlignmentsList(bam_file, param=ScanBamParam(what="seq", flag=flag_sel) )
  close(bam_file)
...
umi_duplicate_comparison <- function(data_names, genome, chr_name_list, sample_to_RE_table, RE_info, sample_to_full_digestion, bad_regions, count_window=c((-200):(-20)), output_file="") {
  
  cat("", file=output_file) # clear file
  
  count_list <- count_fragments_at_cut_sites(data_names, genome, sample_to_RE_table, RE_info, sample_to_full_digestion, bad_regions, count_window )
  
  tmp_sample_to_RE_table <- sample_to_RE_table
  rownames(tmp_sample_to_RE_table) <- paste0(rownames(tmp_sample_to_RE_table), "_duplicate")
  duplicate_count_list <- count_fragments_at_cut_sites( paste0(data_names, "_duplicate"), genome, tmp_sample_to_RE_table, RE_info, sample_to_full_digestion, bad_regions, count_window )
  ...
  for(genome_type in names(chr_name_list)) {
    ...
    tmp_sum <-  function (x) sum(unlist(x[chrs]))
    
    helper_print_table(cbind("unique"= sapply(count_list$fragment_count, tmp_sum),
                             "duplicates"= sapply(duplicate_count_list$fragment_count, tmp_sum),
                             "fraction"=sapply(duplicate_count_list$fragment_count, tmp_sum) /  (sapply(duplicate_count_list$fragment_count, tmp_sum) + sapply(count_list$fragment_count, tmp_sum))
    ), file=output_file)
    
    cat(paste0("\n\n#### Fragments at cut site:\n"), append=TRUE)
    helper_print_table(data.frame("unique"=mapply( '+', sapply(count_list$cut_starts, tmp_sum), sapply(count_list$cut_ends, tmp_sum)),
                                  "duplicates"=mapply( '+', sapply(duplicate_count_list$cut_starts, tmp_sum), sapply(duplicate_count_list$cut_ends, tmp_sum)),
                                  "fraction"=mapply( '+', sapply(duplicate_count_list$cut_starts, tmp_sum), sapply(duplicate_count_list$cut_ends, tmp_sum)) / (mapply( '+', sapply(duplicate_count_list$cut_starts, tmp_sum), sapply(duplicate_count_list$cut_ends, tmp_sum))+mapply( '+', sapply(count_list$cut_starts, tmp_sum), sapply(count_list$cut_ends, tmp_sum)))
    ), file=output_file)
  }
...
# this function is not maintained
any_duplicate_comparison <- function(data_names, genome, chr_name_list, sample_to_RE_table, RE_info, sample_to_full_digestion, bad_regions, count_window=c((-200):(-20)), output_file="") {
  ...
  for(name in data_names) {
    ...
    for(chr in names(bam)) {
      bam[[chr]] <- plyr::count(bam[[chr]])
    }
    
    save(bam, file=paste0(name, "_no_duplicates", ".RData"))
  }
  
  count_list <- count_fragments_at_cut_sites(data_names, genome, sample_to_RE_table, RE_info, sample_to_full_digestion, bad_regions, count_window )
  
  tmp_sample_to_RE_table <- sample_to_RE_table
  rownames(tmp_sample_to_RE_table) <- paste0(rownames(tmp_sample_to_RE_table), "_no_duplicates")
  no_duplicate_count_list <- count_fragments_at_cut_sites( paste0(data_names, "_no_duplicates"), genome, tmp_sample_to_RE_table, RE_info, sample_to_full_digestion, bad_regions, count_window )
  ...
  for(genome_type in names(chr_name_list)) {
    ...
    tmp <- cbind("unique"= sapply(no_duplicate_count_list$fragment_count, tmp_sum),
                 "duplicates"= sapply(count_list$fragment_count, tmp_sum)-sapply(no_duplicate_count_list$fragment_count, tmp_sum),
                 "fraction"=(sapply(count_list$fragment_count, tmp_sum)-sapply(no_duplicate_count_list$fragment_count, tmp_sum)) /  (sapply(count_list$fragment_count, tmp_sum))
    )
    
    helper_print_table(tmp, file=output_file)
    
    
    cat(paste0("\n\n#### Fragments at cut site:\n"), file=output_file)
    
    tmp <- data.frame("unique"=mapply( '+', sapply(no_duplicate_count_list$cut_starts, tmp_sum), sapply(no_duplicate_count_list$cut_ends, tmp_sum)),
                      "duplicates"=mapply( '+', sapply(count_list$cut_starts, tmp_sum), sapply(count_list$cut_ends, tmp_sum))-mapply( '+', sapply(no_duplicate_count_list$cut_starts, tmp_sum), sapply(no_duplicate_count_list$cut_ends, tmp_sum)),
                      "fraction"=(mapply( '+', sapply(count_list$cut_starts, tmp_sum), sapply(count_list$cut_ends, tmp_sum))-mapply( '+', sapply(no_duplicate_count_list$cut_starts, tmp_sum), sapply(no_duplicate_count_list$cut_ends, tmp_sum))) / (mapply( '+', sapply(count_list$cut_starts, tmp_sum), sapply(count_list$cut_ends, tmp_sum))+mapply( '+', sapply(count_list$cut_starts, tmp_sum), sapply(count_list$cut_ends, tmp_sum)))
    )
...
plot_counts_vs_nn_distance_alternative <- function(cut_counts_df_list, chr_name_list, chr_sizes, genome_names, plot_folder = window_folder, outlier_quantile=0.99, max_dist=600) {
  ...
  for(name_X in grep("_X$", names(cut_counts_df_list), value = TRUE)) {
    ...
    for(genome_type in names(chr_name_list)) {
      ...
      png(paste0(plot_folder, "cut_counts_vs_nn_distance_", name, "_", genome_type, "_alternative.png"), res=160, width=2000, height=3000)
      par(mfcol=c(6, 4), oma=c(0, 0, 0, 0), mai=c(0.65, 0.65, 0.1, 0.1), cex.lab = 1.5)
      ...
      helper_plot <- function(...) {
        plot(..., pch=20, col=adjustcolor(palette()[1], alpha.f=min(0.1, 0.1*5000/length(nn_dist_upstream))),
             xlim=c(0, max_dist))
      }
      helper_plot_binned_mean <- function(x, y) {
        x_binned <- round(x/20)*20
        temp_df <- aggregate(y, list(x_binned), mean, na.rm = TRUE)
        colnames(temp_df) <- c("binned_distance", "mean_count")
        lines(temp_df$binned_distance, temp_df$mean_count, col="red", lwd=2.5)
      }
      ...
      for(name_X_1 in c(name_X, name_1)) {
        ...
        for(type in c("starts_plus", "ends_plus", "uncut_plus", "starts_minus", "ends_minus", "uncut_minus")) {
          ...
          helper_plot(nn_dist_upstream, counts, ylab=paste0(ifelse(name_add=="X", "cut", "all cut"), " sample ", gsub("_", " ", type)),
                      xlab="nn distance upstream")
          helper_plot_binned_mean(nn_dist_upstream, counts)
          
          helper_plot(nn_dist_downstream, counts, ylab=paste0(ifelse(name_add=="X", "cut", "all cut"), " sample ", gsub("_", " ", type)),
                      xlab="nn distance downstream")
          helper_plot_binned_mean(nn_dist_downstream, counts)
        }
      }
...
plot_count_histograms_alternative <- function(cut_counts_df_list, chr_name_list, genome_names, plot_folder = window_folder, outlier_quantile=0.99, name_add="") {
  ...
  for(name_X in grep("_X$", names(cut_counts_df_list), value = TRUE)) {
    ...
    for(genome_type in names(chr_name_list)) {
      ...
      png(paste0(plot_folder, "counts_histogram_", name, "_", genome_type, "_alternative.png"), res=144, width=1024, height=2048)
      par(mfcol=c(6, 2), oma=c(0, 0, 0, 0))
      ...
      for(name_X_1 in c(name_X, name_1)) {
        ...
        for(type in c("starts_plus", "starts_minus", "ends_plus", "ends_minus", "uncut_plus", "uncut_minus")) {
          ...
          helper_hist(counts, xlab="counts at site",
                      main=gsub("_", " ", paste0(ifelse(name_add=="X", "cut", "all cut"), " sample ", type)))
        }
      }
      dev.off()
    }
...
print_occs_from_mean_counts <- function(cut_statistics, normalization_method = "norm_cuts", output_file = "") {  # new 2018-02-27
  ...
  accs_occs_main$acc_bg_corrected <- (cuts_X - bg) / (cuts_1 - bg)
  accs_occs_main$acc_cut_uncut_untreated <- cuts_X / (cuts_X + uncuts_X)
  accs_occs_main$acc_cut_uncut_bg_corrected <- (cuts_X - bg) / (cuts_X - bg + uncuts_X)
  
  accs_occs_main$occ_bg_untreated <- 1 - accs_occs_main$acc_bg_untreated
  accs_occs_main$occ_bg_corrected <- 1 - accs_occs_main$acc_bg_corrected
  accs_occs_main$occ_cut_uncut_untreated <-  1- accs_occs_main$acc_cut_uncut_untreated
  accs_occs_main$occ_cut_uncut_bg_corrected <- with(accs_occs_main, 1-acc_cut_uncut_bg_corrected)
...
calc_accessibilities <- function(cut_counts_df_list, bg_counts_df_list, count_window_list, background_method, chr_name_list, acc_max = 1.5,
  ...
  for(name_X in grep("_X$", names(cut_counts_df_list), value = TRUE)) {
    ...
    norm_reads_X = sum(append_cut_count_vectors(df_X_norm), na.rm=TRUE)
    norm_reads_1 = sum(append_cut_count_vectors(df_1_norm), na.rm=TRUE)
    if(norm_reads_X == 0 & norm_reads_1 == 0) {
      scale_1_to_X <- NaN
    } else {
      if(norm_reads_X < 1000 | norm_reads_1 < 1000) {
        print(paste("Some norm counts smaller than 1000, scaling between _1 and _X sample may be inaccurate:", name))
      }
      scale_1_to_X <- norm_reads_X / norm_reads_1
    }
    ...
    if(background_method == "corrected") {  # applies only to cut-allcut method!
    ...
    if(is.finite(scale_1_to_X) && ( any(shear_prob_enzyme_vec > 1) || any(shear_prob_enzyme_vec < 0) )) {
    ...
    # CUT-UNCUT METHOD:
    # here background is always corrected by use of cut_coverage_ratio_bg
    ...
    calc_acc_from_cut_uncut_ratio <- function(cut_uncut_ratio, cut_coverage_ratio_bg, window_length, uncut_loss_correction) {
      if(!is.finite(cut_coverage_ratio_bg)) {
        ...
        cut_uncut_ratio_bg <- cut_coverage_ratio_bg / (1 - cut_coverage_ratio_bg) / uncut_loss_correction  # equals \sigma in supplemental material
      }
...
plot_promoter_regions <- function(occs_aligned, alignment_window, plot_folder, occ_limits = c(-0.25, 1.25)) {  # checked 2018-02-14
...
get_results_dataframe <- function(RE_run, method_name, min_coverage, num_digits = 2) {
  ...
  if(method_name == "window_limit_times_1_max_length_500_close_distances_200_300_background_corrected") {
    load(paste0("../", RE_run, "/analysis_results/window_limit_times_1_max_length_500/close_distances_200_300/background_corrected/occs_df_list.RData"))  # reload occs_df_list with low coverage sites
  } else {
...
calc_deviation_from_test_runs <- function(accs_df_list, background_folder, name_add = "", min_coverage, enzyme = "combined") {
  ...
  if(enzyme != "combined") {
    accs_df_list <- accs_df_list[grepl(enzyme, names(accs_df_list))]
  }
  calibration_samples_accs <- as.numeric(str_match(names(accs_df_list), "_([0-9]*)_pct_cut")[,2]) / 100
...
run_function_parallel <- function(data_names, f, core_factor = 1, single_core_to_debug = False, unlist_after=TRUE) {
  ...
  if(!single_core_to_debug) {
    ...
    result_list <- lapply(data_names, f)
  }
...
compare_two_RE_samples <- function(name_list, RE_sample_results, occ_type_1 = "occ_X_1", occ_type_2 = "occ_X_1", plot_folder) {
  ...
  run_1 <- strsplit(name_list[[1]], "|", fixed = TRUE)[[1]][1]
  name_1 <- strsplit(name_list[[1]], "|", fixed = TRUE)[[1]][2]
  sample_1 <- strsplit(name_list[[1]], "|", fixed = TRUE)[[1]][3]
  run_2 <- strsplit(name_list[[2]], "|", fixed = TRUE)[[1]][1]
  name_2 <- strsplit(name_list[[2]], "|", fixed = TRUE)[[1]][2]
  sample_2 <- strsplit(name_list[[2]], "|", fixed = TRUE)[[1]][3]
  
  occs_df_1 <- load_RE_occs_df(run_1, name_1)
  occs_df_2 <- load_RE_occs_df(run_2, name_2)
  
  occs_df_1$occ_chosen <- occs_df_1[, occ_type_1]
  occs_df_2$occ_chosen <- occs_df_2[, occ_type_2]
  
  occ_limits <- c(0, 1)
  occs_df_1$occ_chosen <- set_outliers_to_limits(occs_df_1$occ_chosen, occ_limits)
  occs_df_2$occ_chosen <- set_outliers_to_limits(occs_df_2$occ_chosen, occ_limits)
  
  occs_df_1_chr_list <- split(occs_df_1, as.character(occs_df_1$chr))
  occs_df_2_chr_list <- split(occs_df_2, as.character(occs_df_2$chr))
  
  RE_1 <- sub("-HF", "", subset(RE_sample_results, file_name == name_1 & run == run_1)[, "RE"], fixed = TRUE)
  RE_2 <-  sub("-HF", "", subset(RE_sample_results, file_name == name_2 & run == run_2)[, "RE"], fixed = TRUE)
  RE_info <- read.table("../RE_info.txt")
  p_1 <- as.character(RE_info[RE_1, "recognition_site"])  # gives NA for double enzyme samples
  p_2 <- as.character(RE_info[RE_2, "recognition_site"])
  
  png(paste0(plot_folder, "/occupancy_scatterplot_", sample_1, "_vs_", sample_2, ".png"), width=3*1024, height=1024, res=1.5*144)
  par(mfrow=c(1, 3))
  
  if(!is.na(p_1) && !is.na(p_2) && (grepl(p_1, p_2) || grepl(p_2, p_1))) {
    
    radius <- 0
    
    occs_1_chr_list <- list()
    occs_2_chr_list <- list()
    
    for(chr in names(occs_df_1_chr_list)) {
      occs_df_1_chr <- occs_df_1_chr_list[[chr]]
      occs_df_2_chr <- occs_df_2_chr_list[[chr]]
      
      sites_1 <- occs_df_1_chr$pos[!is.na(occs_df_1_chr$occ_chosen)]
      sites_2 <- occs_df_2_chr$pos[!is.na(occs_df_2_chr$occ_chosen)]
      
      sites <- intersect(sites_1, sites_2)
      
      occs_1_chr_list[[chr]] <- occs_df_1_chr$occ_chosen[occs_df_1_chr$pos %in% sites]
      occs_2_chr_list[[chr]] <- occs_df_2_chr$occ_chosen[occs_df_2_chr$pos %in% sites]
    }
    
    occs_1 <- unlist(occs_1_chr_list)
    occs_2 <- unlist(occs_2_chr_list)
    ...
    for(chr in names(occs_df_1_chr_list)) {
      ...
      sites_1_with_close_match <- do.call(c, lapply(sites_2, function(s_2) sites_1[abs(sites_1 - s_2) <= radius]))
      sites_2_with_close_match <- do.call(c, lapply(sites_1, function(s_1) sites_2[abs(sites_2 - s_1) <= radius]))
      
      occs_temp_1 <- occs_df_1_chr$occ_chosen[do.call(c, lapply(sites_1_with_close_match, function(pos) which(occs_df_1_chr$pos == pos )))]
      occs_temp_2 <- occs_df_2_chr$occ_chosen[do.call(c, lapply(sites_2_with_close_match, function(pos) which(occs_df_2_chr$pos == pos )))]
      
      
      occs_1_chr_list[[chr]] <- occs_temp_1
      occs_2_chr_list[[chr]] <- occs_temp_2
      
      if(length(occs_1_chr_list[[chr]]) != length(occs_2_chr_list[[chr]])) browser()
    }
  ...
  hist(occs_1, breaks=seq(occ_limits[1], occ_limits[2], 0.025), xlab=sample_1, probability=TRUE, ylim=c(0, 10), main="")
  
  helper_plot(occs_1, occs_2, col="black", alpha.f=0.1, pch=20, xlim=occ_limits, ylim=occ_limits, main=paste0(sample_1, " vs ", sample_2),
              xlab=sample_1, ylab=sample_2)
  
  hist(occs_2, breaks=seq(occ_limits[1], occ_limits[2], 0.025), xlab=sample_2, probability=TRUE, ylim=c(0, 10), main = "")
  
  dev.off()
  
  delta <- mean(abs(occs_1 - occs_2))
  delta_2 <- mean((occs_1 - occs_2)^2)
  mean_diff <- mean(occs_1 - occs_2)
  gamma <- mean(abs(occs_1 - occs_2 - mean_diff))
  gamma_2 <- mean((occs_1 - occs_2 - mean_diff)^2)
  beta <- mean(abs(occs_1/mean(occs_1) - occs_2/mean(occs_2)))
  beta_2 <- mean((occs_1/mean(occs_1) - occs_2/mean(occs_2))^2)
  r_value <- cor(occs_1, occs_2)
  
  RE_RE_comparison_table <- data.frame(run_1=run_1, name_1=name_1, sample_1=sample_1, run_2=run_2, name_2=name_2, sample_2=sample_2,
                                       mean_occ_1 = mean(occs_1), mean_occ_2 = mean(occs_2), radius=radius, num_comparisons=length(occs_1),
                                       delta=delta, delta_2=delta_2, gamma=gamma, gamma_2=gamma_2, beta=beta, beta_2=beta_2, r_value=r_value)
  
  return(RE_RE_comparison_table)
  
}


load_RE_occs_df <- function(run, names, method_folder = "window_limit_times_1_max_length_500/close_distances_200_300/background_corrected/", as_list = FALSE) {
...
calc_and_save_occupancy_map <- function(RE_sample_results, output_folder, occ_type, min_coverage = 0, name_add) {
  
  occs_df_list <- list()
  
  for(this_run in unique(RE_sample_results$run)) {  # load occs_df_lists (these have still all sites in them including low coverage sites)
    this_occs_df_list <- load_RE_occs_df(this_run, subset(RE_sample_results, run == this_run)[, "file_name"], as_list = TRUE)
    names(this_occs_df_list) <- with(subset(RE_sample_results, run == this_run), paste0(nuclei_prep, "_", RE, "_", units, "_", time, "min", add_(buffer), add_(extra_enzyme), "_", run))
    occs_df_list <- c(occs_df_list, this_occs_df_list)
  }
  
  occs_df_list <- lapply(names(occs_df_list), function(name) {
    occs_df <- occs_df_list[[name]]
    colnames(occs_df)[colnames(occs_df) == occ_type] <- "occ"
    occs_df <- occs_df[, c("chr", "enzyme", "pos", "eff_coverage", "occ")]
    occs_df$chr <- sub("chr0", "chr", as.character(occs_df$chr))
    occs_df$enzyme <- as.character(occs_df$enzyme)
    occs_df$chr_pos <- paste0(occs_df$chr, "_", occs_df$pos)
    occs_df$sample <- name
    occs_df$norm_eff_coverage <- occs_df$eff_coverage / mean(occs_df$eff_coverage, na.rm = TRUE)
    occs_df <- occs_df[occs_df$eff_coverage >= min_coverage, ]
    return(occs_df)
  })
  
  names(occs_df_list) <- sapply(occs_df_list, function(occs_df) occs_df$sample[1])
  
  occs_df <- bind_rows(occs_df_list)
  
  mean_occs_df <- aggregate(x = occs_df[, c("occ", "eff_coverage", "norm_eff_coverage")], by = list(occs_df$chr_pos), FUN = function(x) mean(x, na.rm = TRUE))
  colnames(mean_occs_df) <- c("chr_pos", "mean_occ", "mean_eff_coverage", "mean_norm_eff_coverage")
  mean_occs_df$mean_occ[is.nan(mean_occs_df$mean_occ)] <- NA
  
  sd_occs_df <- aggregate(x = occs_df$occ, by = list(occs_df$chr_pos), FUN = function(x) sd(x, na.rm = TRUE))
  colnames(sd_occs_df) <- c("chr_pos", "sd_occ")
  
  sample_num_df <- aggregate(x = occs_df$chr_pos, by = list(occs_df$chr_pos), FUN = length)
  colnames(sample_num_df) <- c("chr_pos", "num_samples")
  
  not_na_df <- aggregate(x = occs_df$occ, by = list(occs_df$chr_pos), FUN = function(x) sum(!is.na(x)))
  colnames(not_na_df) <- c("chr_pos", "num_not_na")
  
  paste_unique <- function(x) {
    paste0(sort(unique(x)), collapse = ", ")
  }
  info_df <- aggregate(x = occs_df[, c("chr", "pos", "enzyme", "sample")], by = list(occs_df$chr_pos), FUN = paste_unique)
  colnames(info_df) <- c("chr_pos", "chr", "pos", "enzymes", "sample_names")
  info_df$pos <- as.integer(info_df$pos)
  info_df$chr <- sub("chr0", "chr", info_df$chr)
  
  occ_map_df <- mean_occs_df
  occ_map_df <- merge(occ_map_df, sd_occs_df)
  occ_map_df <- merge(occ_map_df, sample_num_df)
  occ_map_df <- merge(occ_map_df, not_na_df)
  occ_map_df <- merge(occ_map_df, info_df[, c("chr_pos", "chr", "pos", "enzymes")])
  
  occ_map_df <- occ_map_df[order(occ_map_df$chr, occ_map_df$pos), c("chr", "pos", "enzymes", "mean_occ", "sd_occ", "num_samples", "num_not_na", "mean_eff_coverage", "mean_norm_eff_coverage")]
  rownames(occ_map_df) <- NULL
  
  occ_map_df <- occ_map_df[!is.na(occ_map_df$mean_occ), ]
  
  save(occs_df_list, file = paste0(output_folder, "occs_df_list", name_add, ".RData"))
  save(occ_map_df, file = paste0(output_folder, "occ_map_df", name_add, ".RData"))
...
plot_cut_uncut_length_distributions <- function(name, cut_counts_df_list, count_window_list, RE_info, max_length = 500, max_plot_length = 300, plot_folder, name_add = "") {
  ...
  for(this_enzyme in unique(as.character(with(cut_counts_df_list[[name]], enzyme[chr %in% chr_name_list[["main"]]])))) {  # new 2021-09-19
    site_shift <- RE_info[this_enzyme, "site_shift"]
    cut_reads_list[[this_enzyme]] <- list()
    uncut_reads_list[[this_enzyme]] <- list()
    cut_counts_df <- subset(cut_counts_df_list[[name]], enzyme == this_enzyme & (chr %in% chr_name_list[["main"]]))  # new 2021-09-19
    ...
    cut_reads <- unlist(cut_reads_list[[this_enzyme]])
    uncut_reads <- unlist(uncut_reads_list[[this_enzyme]])
